...
### Fixed
- Shipped optional cloud-provider dependencies in release archives.
- Corrected detached-daemon PID tracking, duplicate starts, and orphan cleanup.
- Hid interactive API-key entry and made embedding-model reset reliable.
- Preserved currency-formatted values during deterministic dataset ingestion.
- Updated release metadata and frontend security dependencies.

## [v1.0.10] - 2026-09-07

### Added
- `wizard update --check` reports whether a newer release is available.
- Managed release installations stage a platform package, verify its published
  SHA-256 checksum, and activate it only after verification while preserving
  the prior package for rollback.
- Production readiness controls add sandbox readiness checks, bounded request
  telemetry, backup lifecycle controls, feature kill switches, scoped cache
  isolation, and fail-safe runtime fallback.
- Hybrid document retrieval now exposes source-aware citations and retrieval
  provenance for grounded answers.
