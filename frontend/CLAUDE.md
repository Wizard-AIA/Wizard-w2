...
- **The chat socket and the stored id must be the same session.** The socket is bound to one session when it connects; REST calls read the stored id on every request. The stored id is the authority (it is where uploads went) and the socket follows it: `createSocketSession` in [lib/session-recovery.ts](lib/session-recovery.ts) decides, `use-chat-stream.ts` executes. A `session` frame adopts its id only when nothing newer is stored, recovery never overwrites an id stored while it was minting, and any change to the stored id (`onSessionIdChange` in `api.ts`, which also hears other tabs through the `storage` event) moves the socket, waiting until no turn or queued interrupt is in flight. Anything that stops a turn must go through `setRunning(false)`, which is what releases a waiting move. A new writer of the stored id needs no extra code, but a new place that opens a chat socket must go through `connect()`.
